...
namespace Stiche_zaubern
{
    public class AIPlayerInTricking : AIPlayerInTrick
    {
        ...
        public AIPlayerInTricking(PlayerInRound player, AIPlayerInRound aiPlayer) : base(player, aiPlayer)
        {
        }

        public void updateAIState(int toGet, int overGuessing, bool beats, List<PlayerInRound> otherPlayers)
        {
            State = calcAIState(toGet, overGuessing, beats, otherPlayers);
        }

        private AIState calcAIState(int toGet, int overGuessing, bool beats, List<PlayerInRound> playersToCome)
        {
            if (toGet <= 0)
            {
            ...
            return toGet == 1 && expectedDanger == 0 && beats ? AIState.CAUTIOUS : toGet > SumGuessing ? AIState.WANTS_TRICK : AIState.SATISFIED;
        }
    }
}