...
namespace Stiche_zaubern
{
    public class Card : IComparable<Card>
    {
        ...
        {
            return new BitmapImage(new Uri("ms-appx:///Assets/" + CardColorExtensions.getColorName(color) + wert + ".jpg"));
        }
        ...
        {
            int rank = color.CompareTo(other.color);
            if (rank != 0)
            {
                return rank;
            }

            rank = wert.CompareTo(other.wert);
            return rank != 0 ? rank : isJuggler() || other.isJuggler() ? (!isJuggler()).CompareTo(!other.isJuggler()) : id.CompareTo(other.id);
        }
    }
