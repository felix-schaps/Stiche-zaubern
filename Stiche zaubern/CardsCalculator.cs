...
namespace Stiche_zaubern
{
    public class CardsCalculations
    {
        public static void seperateJerks(List<Card> cards)
        {
            ...
            _ = cards.RemoveAll(x => x.isJerk() || x.isFairy());
        }
        ...
        public static Card bestCard(List<Card> kartenliste, CardColor followSuite)
        {
            ...
            CardColor trumpColor = ActiveRoundInfo.getTrumpColor();
            if (trumpColor != CardColor.SPECIAL && trumpColor != followSuite)
            {
                if (kartenliste.Any(x => x.color == trumpColor))
                {
                    int maxValue = kartenliste.Where(x => x.color == trumpColor).Max(x => x.wert);
                    return kartenliste.First(x => x.color == trumpColor && x.wert == maxValue);
                }
            }
