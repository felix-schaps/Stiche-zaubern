...
namespace Stiche_zaubern
{
    ...
    {
        public AICardProbCalculatorGuessing(Card card, CardValueInUnknownCards valueInfo, List<PlayerInRound> playersToCome)
        {
    ...
    {
        public AICardProbCalculatorWinning(Card card, CardValueInUnknownCards valueInfo, List<PlayerInRound> playersToCome, AIUnknownCardsHelper helper)
        {
        ...
        private double calculateSimple(Card card, CardValueInUnknownCards valueInfo, List<PlayerInRound> playersToCome)
        {
            int beaten = valueInfo.beaten.Count;
        ...
        private double calculateComplex(Card card, CardValueInUnknownCards valueInfo, List<PlayerInRound> playersToCome, AIUnknownCardsHelper helper)
        {
            double prob = 1.0;
            foreach (PlayerInRound player in playersToCome)
            {
                ...
                if (card.color != ActiveRoundInfo.getTrumpColor())
                {
                    prob *= 1.0 - ((1.0 - probFollow) * helper.getProbHasTrump(player));
                }
                prob *= 1.0 - helper.getProbLaysWizard();
            }
            return prob;
        }
    }
